...
export class LotofacilSpyder {
    private objLoteria: Loteria = {}
    private $: any;

    private readonly logger = new Logger(LotofacilSpyder.name);
   ...
   constructor(html) {
    ...
    valuesDefault(): void{
        try {
            ...
            this.objLoteria.loteria = "LOTOFACIL";
            this.objLoteria.nome = "lotofacil";
        } catch (error) {
            this.logger.error(error);
        }
    }
    ...
    getDezenas(): void{
        try {
            const Dezenas = this.$("div #result-ct").first();
              Dezenas.each((index, element) => {
                const regex = /\d{2}/g;
