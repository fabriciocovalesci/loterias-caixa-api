...
export class MaisMilionariaSpyder {
    private objLoteria: Loteria = {}
    private $: any;
    private concursoCurrent: number;

    private readonly logger = new Logger(MaisMilionariaSpyder.name);
   ...
   constructor(html,  concursoCurrent: number) {
    ...
    handleDezena(number: string): string {
        let _number = parseInt(number);
        let new_number = _number < 10 ? "0" + String(_number) : String(_number);
        return new_number
    }
    ...
    checkConcurso() {
        try {
            const concurso = this.$("form > h1")?.first()?.text()?.match(/(\w*)([0-9])(\w+)/g)[0];
    ...
    valuesDefault(): void{
        try {
            ...
            this.objLoteria.loteria = "MAIS_MILIONARIA";
            this.objLoteria.nome = "maismilionaria";
        } catch (error) {
            this.logger.error(error);
        }
    }
    ...
    getConcurso(): void{
        try {
            const concurso = this.$("form h1")?.first()?.text()?.match(/\w+\d+\s/g)[0];            
    ...
    getDezenas(): void{
        try {
            ...
            const Dezenas = this.$("div.res-ns div.result-ct").first();
              Dezenas.each((index, element) => {
                const regex = /\d{2}/g;
                _dezenas.push(...this.$(element).children('span.clover').text().match(/\d/g));
                
                trevosSorteados = this.$(element).children('span.clover').text().match(/\d/g);
                _dezenas.push(...this.$(element).children('span.circle').text().match(regex));

                if(trevosSorteados.length !== 0) {
                    this.objLoteria.trevosSorteados = trevosSorteados.map(elem => this.handleDezena(elem));
                }
              });
              this.objLoteria.dezenas = _dezenas.map(elem => this.handleDezena(elem));             
        } catch (error) {
            this.logger.error(error);
        }
    }
    ...
    getDezenasByOrdemSorteio(): void{
        try {
            ...
            const Dezenas = this.$("div.result-ct").last();
              ...
              this.objLoteria.dezenasOrdemSorteio = _dezenas.map(elem => this.handleDezena(elem)); 
        } catch (error) {
            this.logger.error(error);
        }
    }
